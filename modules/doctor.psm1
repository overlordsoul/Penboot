...
function Start-PenbootDoctor {
  ...
  if(Test-Path -Path "$($ENV:PENBOOT_WORKPATH)\CONFIG.JSON"){
    ...
    foreach($KeyInFile in ($ConfigFile.Keys)) {
      if($CheckList.Contains($KeyInFile)) {
        #Se o conteúdo for objeto
        if(($ConfigFile[$KeyInFile].Values.Count -gt 0)){
          Write-Host "> $KeyInFile exists and has value $($ConfigFile[$KeyInFile].Keys) = `'$($ConfigFile[$KeyInFile].Values)`' in $($ENV:PENBOOT_WORKPATH)\CONFIG.JSON`n" -BackgroundColor Green -ForegroundColor Black
        }
        #Se não for objeto
        else {
          #TODO: Melhorar essa seção
          Write-Host "$KeyInFile => $($ConfigFile[$KeyInFile])"
        }
      }
      else {
